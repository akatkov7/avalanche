...
public class Player implements Parcelable, Collidable {
	...
	public void draw(Canvas c) {
		...
		RectF localRect = new RectF(playerRect.left, ground - playerRect.top,
				playerRect.right, ground - playerRect.bottom);
	...
	public Collision intersects(RectF collided) {
		...
		if (playerRect.bottom < collided.top
			...
			if (sideSwitched && !(collided instanceof Ground))
				minimumIntersectIndex = Collision.SWITCH;
			...
			float intersectLeft = collided.right - playerRect.left;
			if (intersectBot > 0 && intersectBot < minimumIntersect) {
				minimumIntersectIndex = Collision.TOP;
				minimumIntersect = intersectBot;
			}
			if (intersectRight > 0 && intersectRight < minimumIntersect) {
				minimumIntersectIndex = Collision.RIGHT;
	...
	public void adjustPosition(int deltaT) {
		...
		if (midJump && vy <= 0)
			midJump = false;
		float pytemp = py + vy * (deltaT / 1000.0f)
				+ (-ay * (deltaT / 1000.0f) * (deltaT / 1000.0f));

		// add amount of sideJump from jumping
		float pxtemp = px + (vx + additionalSideJumpVelocity)
				* (deltaT / 1000.0f);
	...
	public void writeToParcel(Parcel parcel, int flags) {
		...
		// idk what to do about paint. just load it regardless in the reading
		// thing?
